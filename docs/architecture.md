...
3. **Vector database** (Chroma, `.chroma/`) — stores document chunks embedded with Gemini embeddings (or `nomic-embed-text` locally when running in Ollama privacy mode).
...
flowchart TD
    ...
    subgraph GRAPH [LangGraph Pipeline]
        ...
        RM -->|sufficient, or retrying after failed generation| GEN
        RM -->|still insufficient, retrieval_count < 3| RM
        RM -->|retrieval_count reaches 3| WS
        WS --> GEN
        GEN --> GG
        GG -->|pass| VT
...
The `routing_outcome` field in the response tells you which path the query took (see `QueryOutcome` in `graph/state.py`):

| Outcome | Meaning |
|---|---|
| `trusted_only_verified` | No web search was needed; answer grounded in vector DB documents and verified against trusted sources |
| `trusted_only_unverified` | No web search was attempted, but the answer could not be verified against trusted sources |
| `web_search_verified` | Web search was used, but the answer was verified against trusted sources |
...
| `PREPARE_RETRY_RETRIEVE` | *(inline `prepare_retry_retrieve()` in `graph.py`)* | Increments `retry_after_generation_count` before looping back to `RETRIEVE_MISSING` (max 2 retries before falling back to web search) |
...
| `FINALIZE` | *(inline `finalize()` in `graph.py`)* | Sets `routing_outcome` and user-facing warning |
...
| `retrieval_count` | `int` | Number of vector-store retrievals so far; caps `RETRIEVE_MISSING` at 3 before falling back to web search |
| `retry_after_generation_count` | `int` | Number of retries after a failed `GRADE_GENERATION`; caps at 2 before falling back to web search |
| `generation_passed_grading` | `bool` | Set by `GRADE_GENERATION`; read by the routing function to decide the next step |
| `routing_outcome` | `str` | Final path taken through the graph |
| `retrieval_warning` | `str` | User-facing warning (language-aware) |
| `privacy_mode` | `bool` | True when running in Ollama (fully local) mode |
...
flowchart TD
    ...
    LOCAL --> DOCLING[Docling HybridChunker\nmax 512 tokens/chunk]
    IAEAURL -->|parse page, fetch PDF| DOCLING
    DIRECT -->|download PDF| DOCLING
    DKURL -->|fetch newest XML| SPLITTER[RecursiveCharacterTextSplitter\n2500 chars/chunk, 200 overlap]

    DOCLING --> EMBED[Gemini Embeddings, batch 200\nor local nomic-embed-text for Ollama]
    SPLITTER --> EMBED
    ...
    subgraph CHROMA [Chroma .chroma/]
        ...
        COL3[(radiation-iaea-ollama\nradiation-dk-law-ollama)]
    end
```

### Key ingestion facts

- **Cloud providers (Gemini, OpenAI, Mistral) share Gemini embeddings** — `GOOGLE_API_KEY` is required for ingestion and retrieval regardless of which provider generates answers.
- **Ollama (privacy mode) uses local `nomic-embed-text` embeddings** — no API key required, but it needs a one-time re-ingestion into separate `-ollama`-suffixed collections; cloud and local collections coexist (`ingestion.get_collection_names`).
- Changing `LLM_PROVIDER` between cloud providers (Gemini / OpenAI / Mistral for *generation*) does **not** require re-ingestion; switching to/from Ollama does.
...
- Local PDFs and registry-fetched IAEA PDFs are chunked with Docling's `HybridChunker` (max 512 tokens/chunk, `nomic-embed-text`-compatible tokenizer); Danish XML content from the registry uses `RecursiveCharacterTextSplitter` (2500 characters/chunk, 200 overlap).
- Collection names must not be renamed without re-ingesting: `radiation-iaea` / `radiation-dk-law` (cloud), `radiation-iaea-ollama` / `radiation-dk-law-ollama` (Ollama).
...
flowchart LR
    ...
    FAC -->|ollama| OLL[langchain-ollama\nllama3.1:8b default, fully local]
...
Ollama mode runs fully locally (no API key, zero data leaves the machine) and also auto-disables LangSmith tracing and web search. See the README's [Privacy Mode](../README.md#privacy-mode-fully-local) section for setup and system requirements.
...
| `GET` | `/config` | Public | Returns `{server_has_llm_key: bool}` — whether the server has at least one LLM API key configured |
